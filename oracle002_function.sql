...
DESC employees; -- 테이블 안의 리스트 유형 확인 


SELECT commission_pct, nvl(commission_pct, 0)-- commission_pct에서 null값이 있다면 0으로 바꿔주기/commission의 원래 값이 바뀌는것은 아니고 출력형태만 바꿔줌
FROM employees;
...
-- null값을 CEO로 바꾸기
SELECT first_name, manager_id, nvl(to_char(manager_id), 'CEO') -- manager_id의 유형이 숫자이니, 문자를 넣어주려면 모든 숫자를 char로 변경해야함
FROM employees;


/*---------------------------------------
일반함수(NULL)
2. nvl2(컬럼, 대체값1, 대체값2) : 컬럼의 값이 null 아니면 대체값1, null이면 대체값2로 출력한다.
----------------------------------------*/
...
-- 전체사원수 , 커미션지급 사원수 , 커미션 미지급 사원수 구하기
-- count(*) => 현재 테이블에 저장되어있는 모든 레코드의 수가 구해짐
SELECT count(*) AS 전체사원수, 
            sum(nvl2(commission_pct, 1, 0)) AS "커미션지급 사원수",
            count(*) -sum(nvl2(commission_pct, 1, 0)) AS "커미션미지급 사원수" -- 전체사원수에서 커미션지급 사원수를 빼기
FROM employees;
...
-- commission_pct가 0.4와 같으면 NULL 값 리턴
-- 비교값에 비교연산자 넣을 수 없음 / 정확하게 일치하는 값만 넣을 수 있음 
...
4.  coalesce(컬럼1, 컬럼2)
    ...
    - 컬럼1, 컬럼2 중 한 개가 Null이면  Null이 아닌 컬럼을 리턴한다.
...
-- 입사일(hire_date) 1-3월이면 '1사분기', 4-6월이면 '2사분기',
-- 7-9월이면 '3사분기' , 10-12월이면 '4사분기'
...
--NULL값과 상관없이 모든 레코드수를 리턴
SELECT count(*)
FROM employees; --107

SELECT count(employee_id) --employee_id는 private key라 null값이 허용이 되지 않음
FROM employees; -- 107
...
-- ORA-00937: not a single-group group function (단일 그룹의 그룹함수가 아님 )
-- count(*)는 결과가 1개이지만 first_name은 107개라 맞지 않음
SELECT first_name/*단순컬럼*/, count(*)/*그룹함수*/
FROM employees;


-- 그룹함수와 단순컬럼을 사용하기 위해서는 단순컬럼을 그룹화해야한다. (GROUP BY)
...
GROUP BY department_id
HAVING department_id <= 50 /* NULL은 비교연산자를 사용할 수 없다 */
ORDER BY department_id;
...
-- HAVING : GROUP BY의 조건
SELECT department_id, count(*) AS "인원수" -- java와 연동할 때 한글은 오류발생 가능성 있음 
FROM employees
GROUP BY department_id --부서별로 그룹짓기
HAVING count(*) <=5  -- 5명 이하
ORDER BY department_id; -- 부서별로 오름차순 정렬
...
-- 부서별 최소급여, 최대급여를 출력하는데 최소급여와 최대급여가 같지 않은 경우에만 
-- 부서별 오름차순으로 출력하시오.
...
--3) 부서별로 커미션을 받는 사원의 수를 구하시오.
   ...
   SELECT department_id, count(commission_pct) AS "커미션 받는 사원의 수"
   FROM employees
...
--10) employees 테이블에서 이름(first_name)의 세번째 자리가 'e'인 직원을 검색하시오.
     ...
     -- instr (데이터, 찾을 문자, 시작위치 , 순서)
      ...
      SELECT first_name
     FROM employees
     WHERE instr(first_name, 'e',1,1)=3; -- 1번째 인덱스부터 첫번째로 검색되는 값이 3이면
     ...
     SELECT first_name
     FROM employees
     WHERE instr(first_name, 'e', 3, 2)=8; --3번째 인덱스부터 두번째로 찾는 e
