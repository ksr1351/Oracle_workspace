...
char - 고정길이 문자를 지정 (ex.휴대폰번호)
number(m) - 정수저장
number(m, n) -실수저장 (n ->소수점자리수)
date - 날짜 저장
datetime - 날짜 시간 저장
*/
...
CREATE TABLE student(
    name varchar2(20), --바이트(영문, 특수문자 -1 byte / 한글 -3byte)
    age number(3), --자릿수
...
무결성 제약조건★★★
  ...
  ★★★(외우기) 무결성 제약조건 5종류: not null, unique, primary key, foreign key, check
...
-- emp06_id_pk시퀀스 삭제
ALTER TABLE emp06
DROP CONSTRAINT emp06_id_pk;

--emp06테이블을 삭제하면 관련 시퀀스도 삭제됨
DROP TABLE emp06;
...
ALTER TABLE emp08
DROP CONSTRAINT emp08_deptno_fk; --제약조건 삭제
...
END;
/
-- dept 1에 대해 업데이트가 발생한 후 begin을 시작하라 / 
...
select keyword from cafe_category where keyword="아기자기";
